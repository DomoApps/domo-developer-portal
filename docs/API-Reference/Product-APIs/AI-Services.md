...
axios.post('https://api.example.com/api/ai/v1/text/summarize', {
...
| `temperature` | number (double) | No | Controls randomness in the model's output. Lower values make the output more deterministic. |
| `maxTokens` | integer (int32) | No | The maximum number of tokens to generate in the response. |
...
| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `template` | string | No | The template string used in the prompt. |
...
axios.post('https://example.com/api/ai/v1/text/sql', requestBody, {
  headers: {
    'Content-Type': 'application/json'
  }
})
...
response = requests.post(
    'https://example.com/api/ai/v1/text/sql',
    json=data,
...
- `prompt`: The initial text input provided to the API.
- `output`: The text generated by the model in response to the prompt.
- `modelId`: Identifier for the model used for generating the text.
...
```javascript
fetch('https://your-api-domain.com/api/ai/v1/text/beastmode', {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json'
  },
...
url = "https://your-api-domain.com/api/ai/v1/text/beastmode"
headers = {
    "Content-Type": "application/json"
}
data = {
...
```bash
curl -X POST https://your-api-domain.com/api/ai/v1/text/beastmode \
-H "Content-Type: application/json" \
-d '{
  "input": "Count distinct products",
  "dataSourceSchema": {
...
response = requests.post(
    'https://api.example.com/api/ai/v1/messages/tools',
    headers={'Content-Type': 'application/json'},
...
```javascript
fetch('http://localhost/api/ai/v1/messages/chat', {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json'
  },
...
- `content`: An array that contains the AI-generated response text.
- `modelId`: The identifier of the AI model used for generating the response.
- `isCustomerModel`: A boolean indicating if a custom model was used.
- `sessionId`: The session identifier associated with this AI interaction.
- `requestId`: The request identifier for tracking the specific request.
- `stopReason`: The reason why the AI stopped generating output.
...
response = requests.post(
    'https://example.com/api/ai/v1/image/text',
    json={
        'image': {
            'data': '<base64 string>',
            'type': 'base64',
...
- `prompt`: The prompt used for image analysis.
- `output`: The description or text extracted from the image.
...
| `dimensions`        | integer (int32) | No       | Specifies the number of dimensions for the resulting embeddings. This parameter helps define the size of the generated embedding vectors.   |
...
axios.post('https://api.example.com/api/ai/v1/embedding/text', requestBody, {
  headers: {
    'Content-Type': 'application/json'
  }
})
...
| Parameter           | Type            | Required | Description                                                                                                                           |
|---------------------|-----------------|----------|---------------------------------------------------------------------------------------------------------------------------------------|
| `input`             | array           | No       | The input images to embed. This should be an array of image objects with base64 encoded data and media type (e.g., "image/png").      |
...
| `dimensions`        | integer (int32) | No       | The number of dimensions for the output embedding vector.                                                                             |
| `modelConfiguration`| object          | No       | Additional model-specific configuration parameter key-value pairs.                                                                    |
| `requestId`         | string (uuid)   | No       | A unique identifier for tracking the request, returned in logs for reference.                                                         |
