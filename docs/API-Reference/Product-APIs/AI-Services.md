...
- `prompt`: The generated prompt used by the model.
- `output`: The SQL query generated based on the input.
- `modelId`: The ID of the model used for generating the query.
...
#### Detailed Explanation for Request Body Fields

- **`input`**: The primary text input for which the service will generate a response. This field is required.
- **`sessionId`**: An optional unique identifier (UUID) for the current AI session. If provided, it will link the request to an existing AI session.
- **`promptTemplate`**: An optional prompt template string containing placeholders for parameters that will be injected before being submitted to the model.
- **`parameters`**: An optional object containing custom parameters for the prompt template. These are key-value pairs that get injected into the prompt template where placeholders match the key names.
- **`model`**: An optional string identifying the model configured for Text Generation. It must be preconfigured by an Admin.
- **`modelConfiguration`**: An optional object containing model-specific configuration settings such as `temperature` and `maxTokens`. These specify behavior and constraints for the text generation process.
- **`system`**: An optional string to specify the system message to use. If not provided, the default message will be used.
- **`temperature`**: An optional number (double) that determines the randomness in the output. Values closer to zero make the output more deterministic.
- **`maxTokens`**: An optional integer specifying the maximum number of tokens to generate in the output.
- **`responseFormat`**: An optional string for specifying a structured output format from the model.
...
| Parameter  | Type   | Required | Description                                                        |
|------------|--------|----------|--------------------------------------------------------------------|
| `template` | string | No       | A template string with placeholders that will be filled with values.|
...
- **`prompt`**: The original text input that was processed.
- **`output`**: The generated text output from the model.
- **`modelId`**: The ID of the model used for text generation.

### Error Responses

| Status Code | Description       | Potential Fields                                                   |
|-------------|-------------------|-------------------------------------------------------------------|
| `403`       | Forbidden         | **`error`** - Detailed explanation of why access is restricted.   |
| `409`       | Conflict          | **`error`** - Description of the conflict that occurred.          |
...
fetch('https://yourserver.com/api/ai/v1/text/beastmode', {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json'
  },
...
```python
# Python request example using requests package
import requests

url = 'https://yourserver.com/api/ai/v1/text/beastmode'
headers = {
    'Content-Type': 'application/json'
}
data = {
...
curl -X POST https://yourserver.com/api/ai/v1/text/beastmode \
     -H "Content-Type: application/json" \
     -d '{
  "input": "Count distinct products",
  "dataSourceSchema": {
...
```javascript
fetch('https://your.api.base.url/api/ai/v1/messages/tools', {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json',
  },
...
url = 'https://your.api.base.url/api/ai/v1/messages/tools'
headers = {'Content-Type': 'application/json'}
data = {
    "input": [
        {
                ...
                "properties": {
                    "categories": {
                        ...
                        "items": {
                            "type": "string",
                            "enum": [
                                "coats & jackets", "accessories", "tops", "jeans & trousers",
                                "skirts & dresses", "shoes"
                            ]
                        }
                    },
                    "colors": {
                        ...
                        "items": {
                            "type": "string",
                            "enum": [
                                "black", "white", "brown", "red", "blue", "green", "orange", "yellow",
                                "pink", "gold", "silver"
                            ]
                        }
                    },
...
```bash
curl -X POST 'https://your.api.base.url/api/ai/v1/messages/tools' \
-H 'Content-Type: application/json' \
-d '{
  "input": [
    {
...
response = requests.post(
    'https://api.example.com/api/ai/v1/messages/chat',
    headers={'Content-Type': 'application/json'},
    json={
        "input": [
            {
...
fetch('https://api.example.com/api/ai/v1/image/text', {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json',
  },
...
url = 'https://api.example.com/api/ai/v1/image/text'
headers = {'Content-Type': 'application/json'}
data = {
  "image": {
    "data": "<base64 string>",
...
curl -X POST https://api.example.com/api/ai/v1/image/text \
-H "Content-Type: application/json" \
-d '{
  "image": {
    "data": "<base64 string>",
...
| `dimensions`        | integer (int32) | No       | The dimensions of the output embedding vector.                                                   |
...
|-------------|-----------------------|------------------|
| `403`       | Forbidden             | ```json<br>{<br> "error": "Access denied",<br> "message": "You do not have permission to access this resource."<br>}<br>``` |
| `409`       | Conflict              | ```json<br>{<br> "error": "Conflict",<br> "message": "A conflicting operation is already in progress."<br>}<br>``` |

### Additional Information

- **Model ID Explanation:** The `modelId` in the response indicates the specific model used to generate the embeddings. It provides details about the version and configurations applied during the embedding process.
...
fetch('/api/ai/v1/embedding/image', {
  ...
  body: JSON.stringify({
    input: ["<base64 encoded image 1>", "<base64 encoded image 2>"],
...
url = "http://your-api-url.com/api/ai/v1/embedding/image"
...
```bash
curl -X POST http://your-api-url.com/api/ai/v1/embedding/image \
-H "Content-Type: application/json" \
-d '{
  "input": ["<base64 encoded image 1>", "<base64 encoded image 2>"],
  "model": "example-model-id",
