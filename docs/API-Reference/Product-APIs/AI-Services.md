...
fetch('https://example.com/api/ai/v1/text/sql', {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json'
  },
...
- `prompt`: The constructed SQL query prompt.
- `output`: The generated SQL query.
- `modelId`: The model ID used to generate the SQL.
...
- `modelId`: The ID of the model used for generation.
...
| `toolChoice`         | [ToolChoice](#schema-toolchoice) | No       | Details which tool to use, when multiple options are available.             |
...
The `ToolChoice` object allows for specifying preferences among available tools when more than one option is present.

| Parameter                | Type    | Required | Description |
|--------------------------|---------|----------|-------------|
| `type`                   | string  | No       | Specifies the selection strategy for tool usage, such as 'AUTO'. |
| `name`                   | string  | No       | The name of the specific tool to be used, if applicable. |
...
fetch('http://example.com/api/ai/v1/messages/tools', {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json'
  },
        ...
        parameters: {
          ...
          properties: {
            categories: {
              ...
              items: {
                type: 'string',
                enum: ['coats & jackets', 'accessories', 'tops', 'jeans & trousers', 'skirts & dresses', 'shoes']
              }
            },
            colors: {
              ...
              items: {
                type: 'string',
                enum: ['black', 'white', 'brown', 'red', 'blue', 'green', 'orange', 'yellow', 'pink', 'gold', 'silver']
              }
            },
            keywords: {
...
url = 'http://example.com/api/ai/v1/messages/tools'
headers = {
    'Content-Type': 'application/json'
}
body = {
...
curl -X POST http://example.com/api/ai/v1/messages/tools \
     -H "Content-Type: application/json" \
     -d '{
           "input": [
             {
...
|-------------|-------------|----------------------------------------------------------------------------------------|
| `403`       | Forbidden   | ```json { "error": "Access is forbidden due to invalid credentials." } ```             |
| `409`       | Conflict    | ```json { "error": "Request could not be processed due to a conflict with current state." } ```   |
...
- `embeddings`: An array containing the generated embeddings for the given input text.
...
| Parameter           | Type             | Required | Description                                                                        |
|---------------------|------------------|----------|------------------------------------------------------------------------------------|
| `input`             | array of objects | No       | An array of objects representing the input images to embed. Each object must contain the base64 string of the image data. |
...
| `dimensions`        | integer (int32)  | No       | Specifies the dimensions of the output embeddings.                                 |
| `modelConfiguration`| object           | No       | Additional model-specific configuration parameter key-value pairs.                 |
...
axios.post('https://api.example.com/api/ai/v1/embedding/image', {
  input: [
    {
      data: '<base64 string>',
      type: 'base64',
  ...
  dimensions: 512,
  modelConfiguration: {
    // Add key-value pairs for model configuration
  },
  requestId: '5f1d7f37-1234-4d57-b9e8-5a6345b2b123'
}, {
  headers: {
    'Content-Type': 'application/json'
  }
...
- `embeddings`: An array where each element is an array of numbers representing the embedding of each input image.
- `modelId`: A string reflecting the ID of the model used to generate the embeddings.
