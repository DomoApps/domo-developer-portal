...
- `prompt`: The generated prompt with the included data source schema and input text.
- `output`: The generated SQL query.
- `modelId`: The ID of the model used for generating the SQL query.
...
- `prompt`: The input text requested for generation.
- `output`: The generated response from the AI model.
...
axios.post('https://api.example.com/api/ai/v1/text/beastmode', requestBody, {
  headers: {
    'Content-Type': 'application/json'
  }
})
...
- `prompt`: The SQL prompt generated from the input and data source schema.
- `output`: The SQL calculation derived from the prompt.
- `modelId`: Identifier of the model used for processing the request.
...
- `content`: The message response generated by the AI, in this example explaining why the sky is blue.
- `modelId`: The identifier of the AI model used.
- `isCustomerModel`: Boolean indicating if a custom model was used.
- `sessionId`: The session ID for the current AI interaction.
- `requestId`: A unique identifier for the request.
- `stopReason`: The reason the interaction stopped.
...
| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `template` | string | No | The template for prompt construction. |
...
fetch('https://api.example.com/api/ai/v1/image/text', {
  ...
  body: JSON.stringify({
    input: 'Describe the content of this image.',
    image: {
      data: '<base64 string>',
      type: 'base64',
      mediaType: 'image/png'
...
**Method:** `POST`  
**Endpoint:** `https://api.example.com/api/ai/v1/embedding/text`
...
| `dimensions`        | integer (int32)  | No       | The desired number of dimensions for the returned embedding vectors.                                       |
| `modelConfiguration`| object           | No       | Additional model-specific configuration parameter key-value pairs.                                         |
| `requestId`         | string (uuid)    | No       | A unique identifier for the request to help with request tracking and logging.                             |
...
| Parameter           | Type               | Required | Description                                                                         |
|---------------------|--------------------|----------|-------------------------------------------------------------------------------------|
| `input`             | array              | No       | The input images to embed, provided as an array of image data.                                                          |
| `model`             | string             | No       | The ID of the model to use for Image Embedding. The model must be pre-configured for use by the administrator. |
| `dimensions`        | integer (int32)    | No       | The number of dimensions for the output embeddings. It determines the vector size of the embedding.                      |
| `modelConfiguration`| object             | No       | Additional model-specific configuration parameter key-value pairs.                                                      |
| `requestId`         | string (uuid)      | No       | A unique identifier for the request, used for tracing and logging purposes.                                                   |
...
fetch('https://api.example.com/api/ai/v1/embedding/image', {
  ...
  body: JSON.stringify({
    ...
    model: "domo.domo_ai.domo-embed-image-v1",
    dimensions: 128,
    modelConfiguration: {
      "setting1": "value1",
      "setting2": "value2"
...
curl -X POST https://api.example.com/api/ai/v1/embedding/image \
  -H "Content-Type: application/json" \
  -d '{"input": [{"data": "<base64 string>", "type": "base64", "mediaType": "image/png"}], "model": "domo.domo_ai.domo-embed-image-v1", "dimensions": 128, "modelConfiguration": {"setting1": "value1", "setting2": "value2"}, "requestId": "123e4567-e89b-12d3-a456-426614174000"}'
```
