...
axios.post('https://example.com/api/ai/v1/text/summarize', data, {
  headers: {
...
url = "https://example.com/api/ai/v1/text/summarize"
...
```bash
curl -X POST https://example.com/api/ai/v1/text/summarize \
...
| `temperature`          | number (double)          | No       | Controls randomness in the model's output. Lower values make output more deterministic.                                                                                                      |
| `maxTokens`            | integer (int32)          | No       | The maximum number of tokens to generate in the response.                                                                                                                                    |
| `reasoningConfig`      | [ReasoningConfig](#schema-reasoningconfig) | No       | Configuration for reasoning processes, specifying reasoning efforts or parameters.                                                                                                                                 |

<details> <summary><strong>ParameterizedPromptTemplate Object</strong></summary>

| Parameter | Type   | Required | Description |
|-----------|--------|----------|-------------|
| `template` | string | No       | The prompt template string containing placeholders for parameters. |

</details>

<details> <summary><strong>ReasoningConfig Object</strong></summary>

| Parameter       | Type   | Required | Description |
|-----------------|--------|----------|-------------|
| `reasoningEffort` | string | No       | The level of effort for reasoning tasks, e.g., "high", "medium", "low". |
...
fetch('https://api.example.com/api/ai/v1/text/sql', {
  ...
  body: JSON.stringify({
    ...
    sqlRequestOptions: ["option1", "option2"],
    reasoningConfig: {
...
```javascript
const url = 'https://api.example.com/api/ai/v1/text/generation';
...
- `prompt`: The input text that was used to generate the response.
- `output`: The text generated by the model.
- `modelId`: The identifier of the model used for text generation.
...
const url = 'https://api.example.com/api/ai/v1/text/beastmode';
const data = {
...
| `toolChoice`         | [ToolChoice](#schema-toolchoice) | No       | An object specifying the tool selection criteria and options.                         |
| `validateSchema`     | boolean         | No       | A flag to determine whether to validate the AI response against the provided schema.  |
| `reasoningConfig`    | [ReasoningConfig](#schema-reasoningconfig) | No       | An object that includes configurations to influence the reasoning process used by the AI.|
...
| Parameter              | Type    | Required | Description |
|------------------------|---------|----------|-------------|
| `type`                 | string  | No       | The type of tool selection, such as "AUTO" or "MANUAL". |
| `name`                 | string  | No       | The specific name of the tool, if needed for a manual selection process. |
| `allowParallelToolCalls` | boolean | No     | Indicates if multiple tools can be called in parallel. |
...
| Parameter          | Type   | Required | Description |
|--------------------|--------|----------|-------------|
| `reasoningEffort`  | string | No       | Describes the effort level for reasoning tasks, such as "LOW", "MEDIUM", or "HIGH". |
...
```javascript
// Prepare a POST request using fetch
fetch('https://your-api-url.com/api/ai/v1/messages/tools', {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json'
  },
    ...
    tools: [
      {
        ...
        parameters: {
          type: "object",
          properties: {
            categories: { description: "Categories that could be a match.", type: "array" },
            colors: { description: "Colors that could be a match, empty array if N/A.", type: "array" },
            keywords: { description: "Keywords that should be present in the item title or description.", type: "array" }
          }
        }
      }
    ],
...
```python
# Prepare a POST request using requests
import requests
import json

url = 'https://your-api-url.com/api/ai/v1/messages/tools'
...
```bash
# Prepare a POST request using cURL
curl -X POST https://your-api-url.com/api/ai/v1/messages/tools \
     -H "Content-Type: application/json" \
     -d '{
           "input": [
             {
...
| `maxTokens`         | integer (int32)              | No       | The maximum number of tokens to generate in the response.                   |
| `responseFormat`    | string                       | No       | Model response format specification for structured outputs.                 |
| `reasoningConfig`   | [ReasoningConfig](#schema-reasoningconfig) | No       | Configuration parameters guiding the reasoning logic of the AI.             |
...
| Parameter         | Type   | Required | Description                                                            |
|-------------------|--------|----------|------------------------------------------------------------------------|
| `reasoningEffort` | string | No       | The level of effort or detail the AI should use in formulating a response. |
...
url = 'http://yourapiendpoint.com/api/ai/v1/messages/chat'
headers = {
    'Content-Type': 'application/json'
}
data = {
...
```bash
curl -X POST 'http://yourapiendpoint.com/api/ai/v1/messages/chat' \
-H 'Content-Type: application/json' \
-d '{
  "input": [
    {
...
- `content`: The AI's response content, explaining the given question.
- `modelId`: The identifier of the model used to generate the response.
- `isCustomerModel`: Indicates if the model is a customer-specific one.
- `sessionId`: The session identifier used for the chat.
- `requestId`: The unique request identifier.
- `stopReason`: The reason the AI stopped processing.
...
axios.post('https://yourapi.com/api/ai/v1/image/text', data, {
  headers: {
    'Content-Type': 'application/json'
  }
})
...
- **prompt**: The text prompt submitted to the model.
- **output**: The model's interpreted description of the image.
- **model**: The identifier of the model used for processing.
...
```javascript
fetch('https://your-api-domain.com/api/ai/v1/extract', {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json'
  },
...
url = 'https://your-api-domain.com/api/ai/v1/extract'
...
```bash
curl -X POST 'https://your-api-domain.com/api/ai/v1/extract' -H 'Content-Type: application/json' -d '{
  "input": [
    {
      "type": "TEXT",
      "text": "Contact: Sarah Johnson, email sarah.j@company.com, phone (555) 123-4567"
...
| `dimensions`        | integer (int32)  | No       | Specifies the dimensionality of the output embedding vector. Higher values typically capture more nuanced features.    |
| `modelConfiguration`| object           | No       | Additional model-specific configuration parameter key-value pairs.                                                     |
| `requestId`         | string (uuid)    | No       | A unique identifier for the request, used for tracking and logging purposes. Should be in UUID format.                 |
...
| `dimensions`        | integer (int32)  | No       | Specifies the dimensions of the generated embeddings.                                                                |
| `modelConfiguration`| object           | No       | Additional model-specific configuration parameter key-value pairs.                                                   |
| `requestId`         | string (uuid)    | No       | Unique identifier for the request.                                                                                   |
...
```javascript
fetch('https://api.example.com/api/ai/v1/classification', {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json'
  },
  body: JSON.stringify({
    ...
    labels: [
      { label: 'critical', description: 'System outage or data loss affecting multiple users or business operations' },
      { label: 'high', description: 'Significant functionality issue affecting user productivity' },
      { label: 'medium', description: 'Feature not working as expected but workarounds available' },
      { label: 'low', description: 'Minor issue, cosmetic bug, or feature request' }
    ],
...
url = 'https://api.example.com/api/ai/v1/classification'
headers = {'Content-Type': 'application/json'}
data = {
    "input": [
        {
...
```bash
curl -X POST https://api.example.com/api/ai/v1/classification \
-H "Content-Type: application/json" \
-d '{
  "input": [
    {
...
- `output`: The predicted classification label(s) for the input.
- `classificationType`: The type of classification used (`SINGLE_LABEL` in this case).
- `model`: The ID of the model that performed the classification.
- `isCustomerModel`: Boolean indicating if a custom model was used.
