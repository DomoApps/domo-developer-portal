...
```bash
curl -X POST http://localhost/api/ai/v1/text/summarize \
...
- `prompt`: The summarization prompt sent to the model.
- `output`: The generated summary of the input text.
...
#### sqlRequestOptions
Optional array of SQL request options to control the behavior of the Text-to-SQL AI Service. Example options may include settings for caching, additional metadata, or query optimization.
...
| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `template` | string | No | Template string with placeholders for parameters. |
...
|-------------|-------------|---------|
| `400` | Invalid property in request. | ```json {"error": "Invalid input parameter: modelId."} ``` |
| `403` | Forbidden. | ```json {"error": "Access denied due to insufficient permissions."} ``` |
| `409` | Conflict. | ```json {"error": "The request could not be completed due to a conflict with the current state of the target resource."} ``` |
...
is submitted to the model. A default prompt template is set for each model configured for the Text Generation AI Service. Individual requests can override the  
...
```javascript
fetch('http://example.com/api/ai/v1/text/generation', {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json',
  },
...
```javascript
fetch('https://YOUR_API_BASE_URL/api/ai/v1/text/beastmode', {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json'
  },
...
url = "https://YOUR_API_BASE_URL/api/ai/v1/text/beastmode"
headers = {
    "Content-Type": "application/json"
}
data = {
...
```bash
curl -X POST https://YOUR_API_BASE_URL/api/ai/v1/text/beastmode \
-H "Content-Type: application/json" \
-d '{
  "input": "Count distinct products",
  "dataSourceSchema": {
...
- `prompt`: The MYSQL prompt generated for creating the query.
- `output`: The SQL query generated to perform the requested calculation.
- `modelId`: Identifier of the model that executed the request.
...
response = requests.post('https://api.example.com/api/ai/v1/messages/tools', headers=headers, json=json_data)
...
- `content`: An array containing the model's response in text form.
- `modelId`: The identifier of the AI model used.
- `isCustomerModel`: Indicates if the model is a customer-specific model.
- `sessionId`: The session ID used for this interaction.
- `requestId`: A unique request ID for tracing purposes.
- `stopReason`: Reason why the conversation stopped.
...
axios.post('https://api.example.com/api/ai/v1/image/text', requestBody, {
  headers: {
    'Content-Type': 'application/json'
  }
})
...
| `dimensions`        | integer (int32) | No       | The number of dimensions for the embedding vectors.                                                                 |
...
axios.post('https://api.example.com/api/ai/v1/embedding/text', {
  input: [
...
```json
{
  ...
  "modelId": "model-id-used-for-embedding"
}
```

- `embeddings`: A list of embeddings for the provided input text.
- `modelId`: The ID of the model that was used to generate embeddings.
...
Request body for generating image embeddings.

| Parameter           | Type           | Required | Description                                                                                  |
|---------------------|----------------|----------|----------------------------------------------------------------------------------------------|
| `input`             | array          | Yes      | A list of image objects where each object contains the image data in base64 format.          |
...
| `dimensions`        | integer (int32)| No       | The number of dimensions for the resulting embedding vector.                                 |
| `modelConfiguration`| object         | No       | Additional model-specific configuration parameter key-value pairs.                           |
| `requestId`         | string (uuid)  | No       | A unique identifier for the request that can be used for tracking.                           |
...
fetch('https://api.example.com/api/ai/v1/embedding/image', {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json'
  },
...
- `embeddings`: An array of arrays containing the embedding vectors for the images.
