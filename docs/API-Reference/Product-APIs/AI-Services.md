...
```bash
curl -X POST "https://example.com/api/ai/v1/text/summarize" -H "Content-Type: application/json" -d '{
  "input": "San Francisco, officially the City and County of San Francisco, is a commercial, financial, and cultural center in Northern California. With a population of 808,437 residents as of 2022, San Francisco is the fourth most populous city in the U.S. state of California. The city covers a land area of 46.9 square miles (121 square kilometers) at the end of the San Francisco Peninsula, making it the second-most densely populated large U.S. city after New York City and the fifth-most densely populated U.S. county, behind only four New York City boroughs. Among the 92 U.S. cities proper with over 250,000 residents, San Francisco is ranked first by per capita income and sixth by aggregate income as of 2022."
}'
```
...
const url = 'https://yourapi.com/api/ai/v1/text/sql';
...
url = 'https://yourapi.com/api/ai/v1/text/sql'
headers = {'Content-Type': 'application/json'}
...
```bash
curl -X POST 'https://yourapi.com/api/ai/v1/text/sql' \
...
- `prompt`: The internal representation of the SQL request, including schema details.
...
fetch('https://api.example.com/api/ai/v1/text/generation', {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json',
  },
...
| `promptTemplate` | [ParameterizedPromptTemplate](#schema-parameterizedprompttemplate) | No | A prompt template string that contains placeholders for parameters. |
| `parameters` | object | No | Custom parameters to inject into the prompt template if an associated placeholder is present. |
...
| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `template` | string | ✔ Yes | The core template string containing parameter placeholders. |
| `parameters` | array of strings | No | List of parameters that the template requires. |

</details>

<details>
<summary><strong>ModelConfiguration Object</strong></summary>

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `temperature` | number (double) | No | Controls the randomness of the output. A lower value makes the model more deterministic. |
| `maxTokens` | integer | No | The maximum number of tokens in the generated output. |
| `frequencyPenalty` | number | No | Penalty for repeating tokens. |
| `presencePenalty` | number | No | Penalty for introducing new tokens. |
...
url = "http://example.com/api/ai/v1/text/beastmode"
headers = {
    "Content-Type": "application/json"
}
data = {
...
```bash
curl -X POST http://example.com/api/ai/v1/text/beastmode \
-H "Content-Type: application/json" \
-d '{
  "input": "Count distinct products",
  "dataSourceSchema": {
...
Success response containing the generated prompt and output for the given input and data source schema.
...
| Status Code | Description |
|-------------|-------------|
| `403` | Forbidden. The client does not have access rights to the content, or the account may be lacking required permissions. |
| `409` | Conflict. The request could not be completed due to a conflict with the current state of the target resource, possibly versioning or concurrency issues. |
...
| `toolChoice`           | [ToolChoice](#schema-toolchoice) | No       | Options for how the tool should be invoked, including type and parallel call allowances. |
...
| Parameter               | Type    | Required | Description                                                                     |
|-------------------------|---------|----------|---------------------------------------------------------------------------------|
| `type`                  | string  | No       | Specifies the tool calling strategy, such as 'AUTO' for automatic selection.    |
| `name`                  | string  | No       | The name of the tool to be called.                                              |
| `allowParallelToolCalls`| boolean | No       | Indicates whether multiple tools can be called in parallel.                     |
...
```javascript
fetch('https://example.com/api/ai/v1/messages/tools', {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json'
  },
...
url = 'https://example.com/api/ai/v1/messages/tools'
headers = {'Content-Type': 'application/json'}
...
```bash
curl -X POST https://example.com/api/ai/v1/messages/tools \
  -H "Content-Type: application/json" \
  -d '{
    "input": [
      {
...
```javascript
fetch('https://api.example.com/api/ai/v1/messages/chat', {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json'
  },
...
url = 'https://api.example.com/api/ai/v1/messages/chat'
headers = {
    'Content-Type': 'application/json'
}
body = {
...
```bash
curl -X POST https://api.example.com/api/ai/v1/messages/chat \
-H "Content-Type: application/json" \
-d '{
  "input": [
    {
...
| `dimensions`        | integer (int32)    | No       | The size of the embedding vector to be returned. Defines the number of dimensions in the output embedding. |
...
axios.post('https://api.example.com/api/ai/v1/embedding/text', data, {
  headers: {
    'Content-Type': 'application/json',
  }
})
...
- `embeddings`: An array of arrays, where each inner array represents the embedding vector of the input text.
- `modelId`: The ID of the model used for generating the embeddings.
...
The request body must include the necessary parameters to query the Image Embedding AI Service.

| Parameter           | Type               | Required | Description                                                                                |
|---------------------|--------------------|----------|--------------------------------------------------------------------------------------------|
| `input`             | array              | No       | An array of image data to embed, typically in base64 format.                                |
...
| `dimensions`        | integer (int32)    | No       | The number of dimensions for the image embedding.                                          |
| `modelConfiguration`| object             | No       | Additional model-specific configuration parameter key-value pairs.                          |
| `requestId`         | string (uuid)      | No       | A unique identifier for the request, useful for tracking and logging.                       |

### Request Example

The request example demonstrates how to structure a request using realistic data inputs across different programming languages.
...
In this example, we send an image for embedding using base64 encoding.

```javascript
const fetch = require('node-fetch');

const url = 'https://yourapi.domain/api/ai/v1/embedding/image';
...
const options = {
  ...
  body: JSON.stringify({
    ...
    model: 'image-embedder-v1',
    dimensions: 1024,
    modelConfiguration: { grayscale: true },
    requestId: '123e4567-e89b-12d3-a456-426614174000'
  })
};
...
url = 'https://yourapi.domain/api/ai/v1/embedding/image'
headers = {'Content-Type': 'application/json'}
body = {
    'input': ['<base64 encoded image>'],
    'model': 'image-embedder-v1',
...
```bash
curl -X POST https://yourapi.domain/api/ai/v1/embedding/image \
-H "Content-Type: application/json" \
-d '{
  "input": ["<base64 encoded image>"],
  "model": "image-embedder-v1",
...
```json
{
  ...
  "modelId": "image-embedder-v1"
}
```

- `embeddings`: An array containing the generated image embeddings.
- `modelId`: Identifies the model used to generate the embeddings.
