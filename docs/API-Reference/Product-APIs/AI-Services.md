...
Models with built-in support for system prompts and chat message history do not need to include _system_ or _chatContext_ in the prompt template. Additional parameters can be provided in the `parameters` map as key-value pairs.
...
| `outputWordLength`      | [SizeBoundary](#schema-sizeboundary) | No       | Specifies the desired length of the output summary in words. |
...
| `reasoningConfig`       | [ReasoningConfig](#schema-reasoningconfig) | No       | Configuration related to the level and style of reasoning applied during summarization. |
...
| Parameter | Type   | Required | Description |
|-----------|--------|----------|-------------|
| `template`| string | No       | A customizable template used to configure how input text is incorporated into the summarization prompt. |
...
| `separators`               | array   | No       | A list of characters or strings which can be used to start a new chunk when they appear in the original text. They are ordered from highest to lowest priority. The highest priority separator is used first to split a new chunk. If it is not present, we move to the next separator. |
| `separatorType`            | string  | No       | The type of separator being used. |
...
| Parameter   | Type    | Required | Description |
|-------------|---------|----------|-------------|
| `min`       | integer (int32) | No   | Minimum word count for the summary. |
| `max`       | integer (int32) | No   | Maximum word count for the summary. |

</details>

<details>
<summary><strong>ReasoningConfig Object</strong></summary>

| Parameter        | Type   | Required | Description |
|------------------|--------|----------|-------------|
| `reasoningEffort`| string | No       | Defines the level of reasoning or depth analyses applied in generating the summary. |
...
data = {'input': 'San Francisco, officially the City and County of San Francisco, is a commercial, financial, and cultural center in Northern California. With a population of 808,437 residents as of 2022, San Francisco is the fourth most populous city in the U.S. state of California. The city covers a land area of 46.9 square miles (121 square kilometers) at the end of the San Francisco Peninsula, making it the second-most densely populated large U.S. city after New York City and the fifth-most densely populated U.S. county, behind only four New York City boroughs. Among the 92 U.S. cities proper with over 250,000 residents, San Francisco is ranked first by per capita income and sixth by aggregate income as of 2022.'}
...
| `promptTemplate` | [ParameterizedPromptTemplate](#schema-parameterizedprompttemplate) | No |  A prompt template string that contains placeholders for parameters that will be replaced with parameter values before the prompt is submitted to the model. **Prompt Template Examples** - "${input}" - "${system}\\n${input}" |
| `parameters` | object | No | Custom parameters to inject into the prompt template if an associated placeholder is present. |
...
| `temperature` | number (double) | No | Controls randomness in the model's output. Lower values make output more deterministic. |
| `maxTokens` | integer (int32) | No | The maximum number of tokens to generate in the response. |
| `reasoningConfig` | [ReasoningConfig](#schema-reasoningconfig) | No |  |
...
curl -X POST "https://<instance>.domo.com/api/ai/v1/text/sql" \
  -H "Content-Type: application/json" \
  -d '{"input": "What are my total sales by region?", "dataSourceSchemas": [{"dataSourceName": "Store Sales", "columns": [{"type": "STRING", "name": "product"}, {"type": "LONG", "name": "store"}, {"type": "LONG", "name": "amount"}, {"type": "DATETIME", "name": "timestamp'"}, {"type": "STRING", "name": "region"}]}]}'
```
...
data = {'input': [{'content': [{'text': 'Do you have any blue coats available?',
                         'type': 'TEXT'}],
            'role': 'USER'}],
 'toolChoice': {'type': 'AUTO'},
 'tools': [{'description': 'Searches for products matching certain criteria in '
                           'the database',
            'name': 'get_product_recommendations',
            'parameters': {'additionalProperties': False,
                           'properties': {'categories': {'description': 'categories '
                                                                        'that '
                                                                        'could '
                                                                        'be a '
                                                                        'match',
                                          ...
                                          'colors': {'description': 'colors '
                                                     ...
                                                     'items': {'enum': ['black',
                                                               ...
                                                               'type': 'string'},
                                                     'type': 'array'},
                                          'email': {'description': 'confirmation '
                                                                   'email',
                                                    'format': 'email',
                                                    'type': 'string'},
                                          'id': {'format': 'uuid',
                                          ...
                                          'limit': {'const': 100,
                                                    'description': 'The '
                                                                   ...
                                                                   'user',
                                                    'type': 'integer'},
                                          'phone': {'description': 'user phone '
                                                                   'number',
                                                    'type': 'string'},
                                          'price_range': {'additionalProperties': False,
                                                          'properties': {'max': {'maximum': 200,
                                                                                 'type': 'number'},
                                                                         'min': {'minimum': 100,
                                                                                 'type': 'number'}},
                                                          'required': ['min',
           ...
           {'description': 'Fetches more details about a product',
            'name': 'get_product_details',
            'parameters': {'additionalProperties': False,
                           'properties': {'product_id': {'description': 'The '
                                                                        'ID of '
                                                                        'the '
                                                                        'product '
                                                                        'to '
                           ...
                           'required': ['product_id'],
                           'type': 'object'}},
           {'description': 'Add items to cart when the user has confirmed '
                           'their interest.',
            'name': 'add_to_cart',
            'parameters': {'additionalProperties': False,
                           'properties': {'items': {'items': {'additionalProperties': False,
                                                              'properties': {'product_id': {'description': 'ID '
                                                                                                           'of '
                                                                                                           'the '
                                                                                                           'product '
                                                                                                           'to '
...
data = {'input': [{'content': [{'text': 'Why is the sky blue?', 'type': 'TEXT'}],
            'role': 'USER'}]}
...
Extract text from an image. By default, all text detected in the image is included. The system and input prompt may be modified to describe, classify, or extract specific information from the image.
...
A prompt template is a string that contains placeholders for parameters that will be replaced with parameter values before submission to the model. Each model configured for the Image to Text AI Service has a default prompt template. Individual requests can override the default template by including the `promptTemplate` parameter.
...
| `sessionId`        | string (uuid)                            | No       | The AI session ID. If provided, this request will be associated with the specified AI Session.              |
| `model`            | string                                   | No       | The ID of the model to use for Image to Text processing. The specified model must be configured for the AI Service by an Admin. |
| `system`           | string                                   | No       | The system message for the Image to Text task. If not provided, the default system message will be used. If the model does not support system prompts, this parameter may be included in the prompt template using the "${system}" placeholder. |
| `modelConfiguration` | object                                 | No       | Additional model-specific configuration parameter key-value pairs, e.g., temperature, max_tokens, etc.        |
| `promptTemplate`   | [ParameterizedPromptTemplate](#schema-parameterizedprompttemplate) | No       | A prompt template string with placeholders for parameters that will be replaced with values before submission to the model.<p><h3>Prompt Template Examples</h3><ul><li>"${input}"</li><li>"${system}\n${input}"</li></ul> |
| `parameters`       | object                                   | No       | Custom parameters to inject into the prompt template if an associated placeholder is present.                 |
| `maxTokens`        | integer (int32)                          | No       | The maximum number of tokens to generate in the response.                                                     |
| `temperature`      | number (double)                          | No       | Controls randomness in the model's output. Lower values make output more deterministic.                       |
| `responseFormat`   | string                                   | No       | Model response format specification for structured outputs.                                                   |
| `reasoningConfig`  | [ReasoningConfig](#schema-reasoningconfig) | No     | Configuration for reasoning process such as effort level, e.g., quick, thorough.                              |
...
| Parameter  | Type   | Required | Description                          |
|------------|--------|----------|--------------------------------------|
| `template` | string | No       | The template string containing placeholders for parameters. |
...
| Parameter         | Type   | Required | Description                                                  |
|-------------------|--------|----------|--------------------------------------------------------------|
| `reasoningEffort` | string | No       | The level of effort the AI should apply in reasoning, e.g., "quick". |
...
data = {
    ...
    "input": "Identify objects in the image.",
    "sessionId": "<session-id>",
    "model": "<model-id>"
}
...
curl -X POST "https://<instance>.domo.com/api/ai/v1/image/text" \
  ...
  -d '{
    ...
    "input": "Provide a detailed description of this image.",
    "sessionId": "<session-id>",
    "model": "<model-id>"
  }'
```
...
Request for extracting structured data from unstructured text using the Messages API. The ExtractData service uses LLM structured outputs to extract data conforming to a provided JSON schema from message content input.

| Parameter         | Type                    | Required | Description                                                                   |
|-------------------|-------------------------|----------|-------------------------------------------------------------------------------|
| `input`           | array                   | ✔ Yes    | The input message content to extract data from (text, images, documents, etc.). |
| `outputStructure` | object                  | ✔ Yes    | Map of field names to their FunctionParameter definitions. All fields are treated as required. |
| `instructions`    | string                  | No       | Optional instructions for extraction behavior.                                 |
| `model`           | string                  | No       | The ID of the model to use (optional, uses default if not specified).          |
| `modelConfiguration` | object               | No       | Additional model parameters (temperature, max_tokens, etc.).                  |
| `sessionId`       | string (uuid)           | No       | AI session ID for associating with existing session.                          |
| `reasoningConfig` | [ReasoningConfig](#schema-reasoningconfig) | No | Configuration for reasoning abilities.                                          |
...
| Parameter         | Type   | Required | Description                                                                         |
|-------------------|--------|----------|-------------------------------------------------------------------------------------|
| `reasoningEffort` | string | No       | Level of computational effort used for reasoning. Options may include 'low', 'medium', or 'high'. |
...
```javascript
fetch('https://<instance>.domo.com/api/ai/v1/extract', {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json'
  },
...
url = "https://<instance>.domo.com/api/ai/v1/extract"
headers = {
    "Content-Type": "application/json"
}
data = {'input': [{'text': 'Contact: Sarah Johnson, email sarah.j@company.com, phone '
                    '(555) 123-4567',
            'type': 'TEXT'}],
 'outputStructure': {'email': {'description': 'Email address',
                               'type': 'string'},
...
```bash
curl -X POST "https://<instance>.domo.com/api/ai/v1/extract" \
  -H "Content-Type: application/json" \
  -d '{"input":[{"type":"TEXT","text":"Contact: Sarah Johnson, email sarah.j@company.com, phone (555) 123-4567"}],"outputStructure":{"name":{"type":"string","description":"Full name"},"email":{"type":"string","description":"Email address"},"phone":{"type":"string","description":"Phone number"}}}'
```
...
| `dimensions`         | integer (int32)  | No       | Specifies the number of dimensions for the generated embeddings. This parameter helps to define the size and structure of the embedding output.                                   |
| `modelConfiguration` | object           | No       | Additional model-specific configuration parameter key-value pairs.                                                                                                               |
| `requestId`          | string (uuid)    | No       | A unique identifier for the request, used for tracking and reference purposes.                                                                                                    |
...
data = {'input': ['This is a sample text for generating embeddings.']}
...
curl -X POST "https://<instance>.domo.com/api/ai/v1/embedding/text" \
  -H "Content-Type: application/json" \
  -d '{"input": ["This is a sample text for generating embeddings."]}'
```
...
Generate image embeddings based on the given image inputs.
...
| Parameter            | Type            | Required | Description                                                                                                     |
|----------------------|-----------------|----------|-----------------------------------------------------------------------------------------------------------------|
| `input`              | array           | No       | The input images to embed, structured as an array of objects. Each object should contain `data`, `type`, and `mediaType`. |
...
| `dimensions`         | integer (int32) | No       | Number of dimensions for the output embeddings. It specifies the size of the resulting embedding vectors.       |
| `modelConfiguration` | object          | No       | Additional model-specific configuration parameters, provided as key-value pairs.                                |
| `requestId`          | string (uuid)   | No       | A unique identifier for the request. Useful for tracking and debugging purposes.                                 |
...
**Method:** `POST`  
**Endpoint:** `/api/ai/v1/classification`
...
Request for classifying content into predefined categories using the Messages API. The Classification service uses LLM structured outputs to reliably classify content into one or more predefined categories. The service dynamically generates JSON schemas based on the provided labels to ensure type-safe responses.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `input` | array | ✔ Yes | The input message content to classify (text, images, documents, etc.). |
| `labels` | array | ✔ Yes | The list of possible classification labels with optional descriptions (2-50 labels required). |
| `classificationType` | string | ✔ Yes | The type of classification: SINGLE_LABEL or MULTI_LABEL Allowed values: `SINGLE_LABEL`, `MULTI_LABEL`. |
| `instructions` | string | No | Optional instructions for classification behavior. |
| `model` | string | No | The ID of the model to use (optional, uses default if not specified). |
| `modelConfiguration` | object | No | Additional model parameters (temperature, max_tokens, etc.). |
| `sessionId` | string (uuid) | No | AI session ID for associating with existing session. |
| `reasoningConfig` | [ReasoningConfig](#schema-reasoningconfig) | No |  |
...
```javascript
fetch('https://<instance>.domo.com/api/ai/v1/classification', {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json'
  },
...
url = "https://<instance>.domo.com/api/ai/v1/classification"
headers = {
    "Content-Type": "application/json"
}
data = {'classificationType': 'SINGLE_LABEL',
 'input': [{'text': "URGENT: Our production dashboard is down and we can't "
                    'access any of our KPI data. This is affecting our entire '
                    'sales team and we need this resolved immediately!',
            'type': 'text'}],
 'labels': [{'description': 'System outage or data loss affecting multiple '
                            'users or business operations',
             'label': 'critical'},
            {'description': 'Significant functionality issue affecting user '
                            'productivity',
             'label': 'high'},
            {'description': 'Feature not working as expected but workarounds '
                            'available',
             'label': 'medium'},
            {'description': 'Minor issue, cosmetic bug, or feature request',
             'label': 'low'}]}
...
```bash
curl -X POST "https://<instance>.domo.com/api/ai/v1/classification" \
  -H "Content-Type: application/json" \
  -d '{"input":[{"type":"text","text":"URGENT: Our production dashboard is down and we can'\''t access any of our KPI data. This is affecting our entire sales team and we need this resolved immediately!"}],"labels":[{"label":"critical","description":"System outage or data loss affecting multiple users or business operations"},{"label":"high","description":"Significant functionality issue affecting user productivity"},{"label":"medium","description":"Feature not working as expected but workarounds available"},{"label":"low","description":"Minor issue, cosmetic bug, or feature request"}],"classificationType":"SINGLE_LABEL"}'
```
