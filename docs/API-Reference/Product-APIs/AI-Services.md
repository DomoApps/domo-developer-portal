...
```javascript
fetch('https://yourapi.com/api/ai/v1/text/summarize', {
  method: 'POST',
  headers: {
...
```javascript
fetch('https://your-api-domain.com/api/ai/v1/text/generation', {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json',
  },
...
url = 'https://your-api-domain.com/api/ai/v1/text/generation'
...
```bash
curl -X POST https://your-api-domain.com/api/ai/v1/text/generation \
-H "Content-Type: application/json" \
...
- `prompt`: The SQL query prompt generated for the given task.
- `output`: The calculated Beast Mode expression.
...
- `content`: An array of objects containing the type and text of AI-generated messages.
- `modelId`: Identifier of the AI model used.
- `isCustomerModel`: Boolean indicating if a customer-specific model was used.
- `sessionId`: Unique identifier for the session.
- `requestId`: Unique identifier for the request.
- `stopReason`: Reason why the AI stopped processing.
...
- input
- system Models with built-in support for system prompts and chat message history do not need to include *system* or
- *chatContext* in the prompt template.Additional parameters can be provided in the `parameters` map as key-value pairs.
...
axios.post('https://example.com/api/ai/v1/image/text', data, {
  headers: {
    'Content-Type': 'application/json',
  },
})
...
- `prompt`: The input prompt used for analyzing the image.
- `output`: The text description or extracted information from the image.
- `model`: The model used for processing the request.
...
| `dimensions`        | integer (int32)  | No       | Number of dimensions in the resulting embedding vectors. If not specified, defaults to model's configuration. |
| `modelConfiguration`| object           | No       | Additional model-specific configuration parameter key-value pairs.                          |
| `requestId`         | string (uuid)    | No       | A unique identifier for the request, useful for tracing and logging purposes.                |
...
- `embeddings`: A list of lists containing the embedding vectors for the input text.
...
| `409`       | Conflict. There is a conflict with the current state of the target resource. Possibilities include: conflicting model usage, or inconsistent request parameters. Provide clear error messages to guide the user on corrective actions. |
...
| `modelConfiguration`| object         | No       | Additional model-specific configuration parameter key-value pairs.                                                |
| `requestId`         | string (uuid)  | No       | A unique identifier for the request, used for tracking and debugging purposes.                                    |
...
fetch('https://api.example.com/api/ai/v1/embedding/image', {
  ...
  body: JSON.stringify({
    "input": ["<base64_encoded_image_1>", "<base64_encoded_image_2>"],
    "model": "image-embedding-model-v1",
    "dimensions": 128,
    "modelConfiguration": {
      "parameter1": "value1"
    },
...
- `embeddings`: A list of lists containing the generated embeddings for the input image.
