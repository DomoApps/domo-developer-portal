...
fetch('https://example.com/api/ai/v1/text/summarize', {
  method: 'POST',
  headers: {
...
- `prompt`: The prompt template used for the summarization task.
- `output`: The summarized text.
- `modelId`: The ID of the model used for generating the summary.
...
A prompt template is a string that contains placeholders for parameters, which will be replaced with parameter values before the prompt is submitted to the model. A default prompt template is set for each model configured for the Text-to-SQL AI Service. Individual requests can override the default template by including the `promptTemplate` parameter.
...
| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `template` | string | No | The template string defining the prompt with placeholders. |
...
url = 'https://your-base-url.com/api/ai/v1/text/sql'
headers = {'Content-Type': 'application/json'}
body = {
    "input": "What are my total sales by region?",
    "sessionId": "your-session-id",
...
```bash
curl -X POST https://your-base-url.com/api/ai/v1/text/sql \
  -H "Content-Type: application/json" \
  -d '{
    "input": "What are my total sales by region?",
    "sessionId": "your-session-id",
...
- `prompt`: The SQL query generation prompt including schema and input question.
- `output`: The generated SQL query.
- `modelId`: The identifier of the model used in the query generation.
...
| `modelConfiguration`| object                                | No       | Additional model-specific configuration parameter key-value pairs. e.g. temperature, maxTokens, etc.                                           |
...
| Parameter  | Type   | Required | Description |
|------------|--------|----------|-------------|
| `template` | string | No       | The template string that defines how the prompt will be structured and parameterized. |

</details>

### Request Example

Here are examples using various programming languages and including optional parameters to illustrate the usage of different parameters.
...
response = requests.post(
    'https://api.example.com/api/ai/v1/text/generation',
    json={
        "input": "Why is the sky blue?",
        "sessionId": "123e4567-e89b-12d3-a456-426614174000",
        "promptTemplate": "${system} ${input}",
...
- `output`: The generated text output from the AI model.
- `modelId`: The identification of the model used for generating the text.
...
| `columns` | array | No | The columns in the data source. Each column is an object containing the column name and type. |
...
| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `template` | string | No | The template string with placeholders for parameter values. |
...
fetch('https://api.yourdomain.com/api/ai/v1/text/beastmode', {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json',
  },
...
url = "https://api.yourdomain.com/api/ai/v1/text/beastmode"
...
curl -X POST https://api.yourdomain.com/api/ai/v1/text/beastmode \
-H "Content-Type: application/json" \
-d '{
  "input": "Count distinct products",
  "dataSourceSchema": {
...
The successful response will return a JSON object containing the generated Beast Mode calculation and additional details.

| Parameter | Type | Description |
|-----------|------|-------------|
| `prompt` | string | The generated prompt for the model. |
| `output` | string | The resulting Beast Mode calculation. |
| `modelId` | string | The model ID used for processing the request. |
...
url = 'http://api_endpoint_base/api/ai/v1/messages/tools'
headers = {
    'Content-Type': 'application/json'
}
...
```bash
curl -X POST http://api_endpoint_base/api/ai/v1/messages/tools \
-H "Content-Type: application/json" \
-d '{
  "input": [
    {
...
```javascript
// Using fetch to make the POST request to the chat messages endpoint
fetch('https://your.api.base.url/api/ai/v1/messages/chat', {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json'
  },
...
url = 'https://your.api.base.url/api/ai/v1/messages/chat'
headers = {'Content-Type': 'application/json'}
data = {
  "input": [
    {
...
```bash
curl -X POST https://your.api.base.url/api/ai/v1/messages/chat \
-H "Content-Type: application/json" \
-d '{
  "input": [
    {
...
- `content`: Array of response parts.
  - `type`: Type of content, e.g., TEXT.
  - `text`: The actual response text from the AI model.
- `modelId`: The ID of the model used to generate the response.
- `isCustomerModel`: Indicates if a customer-specific model was used.
- `sessionId`: Session identifier correlating to the request.
- `requestId`: Unique identifier for the specific request.
- `stopReason`: Reason why the model stopped generating more content.
...
const data = {
  ...
  sessionId: "03f6e5bb-381e-45fd-a472-a6cadbe91819",
  model: "example-string",
  system: "example-string",
  modelConfiguration: {},
  promptTemplate: { template: "example-string" },
  parameters: {},
  maxTokens: 42,
  temperature: 3.14159,
  responseFormat: "example-string"
};

fetch('https://example.com/api/ai/v1/image/text', {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json',
  },
...
- `prompt`: The prompt used to extract text from the image.
...
```javascript
fetch('http://example.com/api/ai/v1/embedding/text', {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json'
  },
...
url = "http://example.com/api/ai/v1/embedding/text"
headers = {"Content-Type": "application/json"}
data = {
    "input": [
        "This is a sample text for generating embeddings."
...
- **embeddings**: An array of arrays where each inner array represents the embeddings vector for the provided text input.
- **modelId**: The identifier for the model used to generate the embeddings.
...
Request to generate image embeddings.

| Parameter            | Type            | Required | Description                                                                                                                               |
|----------------------|-----------------|----------|-------------------------------------------------------------------------------------------------------------------------------------------|
| `input`              | array           | Yes      | The input images to embed. Must be an array of objects, where each object contains image data and associated metadata.                     |
| `model`              | string          | Yes      | The ID of the model to use for Image Embedding. The specified model must be configured for the Image Embedding AI Service by an Admin.     |
| `dimensions`         | integer (int32) | Yes      | The dimensionality of the embedding space to be used by the model.                                                                        |
| `modelConfiguration` | object          | No       | Additional model-specific configuration parameter key-value pairs.                                                                        |
| `requestId`          | string (uuid)   | Yes      | Unique identifier for the request, used for tracking and logging purposes.                                                                |
...
axios.post('https://your-api-domain.com/api/ai/v1/embedding/image', data, {
  headers: {
    'Content-Type': 'application/json'
  }
})
...
url = 'https://your-api-domain.com/api/ai/v1/embedding/image'
data = {
    "input": [
        {
            "data": "<base64 string>",
...
```bash
curl -X POST https://your-api-domain.com/api/ai/v1/embedding/image \
  -H "Content-Type: application/json" \
  -d '{
        "input": [
          {
...
- `embeddings`: An array of arrays containing the generated image embeddings.
