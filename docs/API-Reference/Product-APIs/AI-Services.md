...
| `system`               | string                                        | No       | The system message to use for the Text Summarization task. If not provided, the default system message will be used.                                 |
...
```javascript
fetch('https://yourapi.example.com/api/ai/v1/text/summarize', {
  method: 'POST',
  headers: {
...
url = 'https://yourapi.example.com/api/ai/v1/text/summarize'
headers = {'Content-Type': 'application/json'}
data = {
  "input": "San Francisco, officially the ...
}
...
```bash
curl -X POST https://yourapi.example.com/api/ai/v1/text/summarize \
...
axios.post('https://yourapi.com/api/ai/v1/text/sql', data, {
  headers: {
    'Content-Type': 'application/json'
  }
})
...
- `prompt`: The generated prompt used to create the SQL query.
- `output`: Returns the SQL query that answers the input question.
- `modelId`: The identifier of the model used to generate the SQL query.
...
| `input`              | string | ✓ Yes    | The input text.                                                                                                                  |
| `sessionId`          | string (uuid) | No       | The AI session ID. If provided, this request will be associated with the specified AI Session.                                   |
| `promptTemplate`     | [ParameterizedPromptTemplate](#schema-parameterizedprompttemplate) | No       | A prompt template string that contains placeholders for parameters that will be replaced before submission to the model.         |
| `parameters`         | object | No       | Custom parameters to inject into the prompt template if an associated placeholder is present.                                    |
| `model`              | string | No       | The ID of the model to use for Text Generation. Must be configured for the Text Generation AI Service by an Admin.               |
| `modelConfiguration` | object | No       | Additional model-specific configuration parameter key-value pairs (e.g., temperature, max_tokens, etc.).                         |
| `system`             | string | No       | The system message for the Text Generation task. Defaults to the system message if not provided. Include in the template if needed. |
| `temperature`        | number (double) | No       | Controls randomness in the model's output. Lower values make output more deterministic.                                          |
| `maxTokens`          | integer (int32) | No       | The maximum number of tokens to generate in the response.                                                                        |
...
| Parameter | Type   | Required | Description |
|-----------|--------|----------|-------------|
| `template` | string | No       | The prompt template. |
...
-->
```javascript
const url = "/api/ai/v1/text/generation";
...
-->
```bash
curl -X POST /api/ai/v1/text/generation \
-H "Content-Type: application/json" \
-d '{
  "input": "Why is the sky blue?"
}'
...
- **content**: An array containing the AI response to the input, structured as a list of message components.
- **modelId**: The identifier of the AI model that was used to generate the response.
- **isCustomerModel**: A boolean indicating if a custom model was used.
- **sessionId**: The unique session identifier associated with the request.
- **requestId**: A unique identifier for the individual request.
- **stopReason**: A string indicating the reason the response generation stopped.
...
{
  const url = '/api/ai/v1/image/text';
  const data = {
    image: {
      data: '<base64 string>',
      type: 'base64',
...
- `prompt`: The prompt used by the model to process the image.
- `output`: The text extracted or generated from the image.
- `model`: The ID of the model used for processing.
...
| `requestId`        | string (uuid)   | No       | A unique identifier for the request to assist in tracking and referencing specific requests.             |
...
fetch('https://yourapi.com/api/ai/v1/embedding/text', {
  ...
  body: JSON.stringify({
    ...
    dimensions: 128, // Optional
    requestId: "your-unique-request-id" // Optional
  })
})
.then(response => response.json())
.then(data => console.log(data));
...
| `dimensions`         | integer (int32)   | No       | The number of dimensions for the generated image embeddings.                                                 |
| `modelConfiguration` | object            | No       | Additional model-specific configuration parameter key-value pairs.                                           |
| `requestId`          | string (uuid)     | No       | A unique identifier for the request, useful for tracing and debugging purposes.                              |
...
fetch('https://api.example.com/api/ai/v1/embedding/image', {
  ...
  body: JSON.stringify({
    ...
    dimensions: 42,
    modelConfiguration: {},
    requestId: 'cfb80770-0b45-49cf-bc45-3c0947a76879',
...
- `embeddings`: An array of numerical values representing the image embeddings.
- `modelId`: The ID of the model used for generating embeddings.
