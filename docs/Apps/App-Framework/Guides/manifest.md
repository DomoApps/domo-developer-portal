...
_Note_: The `id` field is automatically added to the manifest by the `domo` command line tool the first time it is published via `domo publish`.
...
_Note_: If the `field` property is empty and does not contain any `field` objects, then all columns from the DataSet will be returned when the DataSet requests data. Since no field aliases are defined in this case, the original column names are used to index the data.
...
_Note_: If the `fields` property in a DataSet mapping is left empty (i.e. no field objects are defined), then the original column names from the DataSet will be used in place of the field alias.
