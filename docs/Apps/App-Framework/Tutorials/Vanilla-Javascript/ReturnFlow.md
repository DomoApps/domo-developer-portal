...
This starter code uses Domo's [Data API](../../../../API-Reference/Domo-App-APIs/Data-API.md) to get the first 100 records from our DataSet with the alias `returns`.
...
This code will define a new collection named `comments` with three properties: `id`, `user`, and `postBody`. With the `syncEnabled` property set to true, this will also automatically sync our AppDB collection to a DataSet in Domo. For more on AppDB see the [AppDB API Reference](../../../../API-Reference/Domo-App-APIs/AppDB-API.md).
